...
DATA_DIR = "data"
CSV_PATH = os.path.join(DATA_DIR, "translation_en_ar.csv")
MODEL_DIR = os.path.join("results3", "marian_mt_en_ar")
ARTIFACT_PATH = os.path.join(MODEL_DIR, "model_manifest.joblib")

if not os.path.exists(DATA_DIR):
    os.makedirs(DATA_DIR)

# 1. Load dataset (English to Arabic only)
...
for item in train_data:
    ...
    if "en" in translation and "ar" in translation:
        processed_data["source"].append(translation["en"])
        processed_data["target"].append(translation["ar"])

df = pd.DataFrame(processed_data)
df.to_csv(CSV_PATH, index=False)
train_dataset = Dataset.from_pandas(df)

# 2. Load accurate English-to-Arabic base model
